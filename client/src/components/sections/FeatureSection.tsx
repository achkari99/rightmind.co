    ...
    <section className="py-20 md:py-32 overflow-hidden">
      <div className="container mx-auto px-4 md:px-6">
        <div className={`flex flex-col md:flex-row gap-12 md:gap-24 items-center ${isReversed ? "md:flex-row-reverse" : ""}`}>
          ...
          >
            <div className="text-primary font-bold tracking-wide uppercase text-sm mb-4">{eyebrow}</div>
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-foreground mb-6">{title}</h2>
          ...
          >
            <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-border bg-muted/10 p-2 md:p-4">
              <div className="bg-white rounded-xl overflow-hidden shadow-sm">
                <img src={resolvedImage} alt={imageAlt} className="w-full h-auto object-cover" />
              </div>
              <div
                className={`absolute ${isReversed ? "-bottom-6 -left-6" : "-bottom-6 -right-6"} bg-white p-4 rounded-lg shadow-xl border border-border/50 max-w-[200px] hidden md:block animate-bounce duration-[3000ms]`}
              >
